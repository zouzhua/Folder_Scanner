...
namespace Folder_Scanner
{
    public partial class Form1 : Form
    {
        ...
        {
            //MessageBox.Show(Last_Node.ToolTipText);
            string Item_Name = "";
            if (listView1.SelectedItems.Count > 0)
                Item_Name = listView1.SelectedItems[0].Text;
            Process process = new Process();
            process.StartInfo.FileName = "explorer.exe";
            if (Item_Name == "..")
                process.StartInfo.Arguments = @"/select," + Last_Node.ToolTipText;
            else if (Item_Name == "")
                process.StartInfo.Arguments = Last_Node.ToolTipText;
            else
                process.StartInfo.Arguments = @"/select," + Last_Node.ToolTipText + "\\" + Item_Name;
            process.Start();
        }
